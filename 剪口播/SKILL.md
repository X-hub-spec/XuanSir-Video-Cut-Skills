...
<!--
input: 视频文件 (*.mp4) 或 multicam_sources.json
output: subtitles_words.json、auto_selected.json、review.html、video.mp4(符号链接)、multicam_project.json(多机位)
pos: 转录+识别，到用户网页审核为止
...
> 本地 Whisper MLX / 火山引擎转录 + AI 口误识别 + 网页审核

## 多机位模式 v1

当用户明确要处理三机位、多视频轨、多音频轨或“多机位模式”时，使用多机位流程。首版约束：

- 同场同步素材为默认场景。
- 说话人通过音频轨命名和 `speakerId` 标注，不做自动声纹识别。
- 支持在审核页修改说话人名称、素材 offset、预览机位。
- 支持段落上移/下移，语义是全局时间区间重排。
- 重点导出多轨 FCPXML，不直接渲染 mp4。
...
1. 提取音频 (ffmpeg)
    ↓
2. 本地 Whisper MLX 转录（默认推荐）或上传获取公网 URL 后用火山引擎 API 转录
    ↓
...
```
0. 准备 multicam_sources.json
    ↓
1. node scripts/prepare_multicam_project.js multicam_sources.json
    ↓
2. 生成 multicam_project.json 和 media/ 素材链接
    ↓
3. node scripts/generate_review.js multicam_project.json
    ↓
4. node scripts/review_server.js 8899
    ↓
5. 网页审核：说话人标注、画线删除、段落移动、offset 校准
    ↓
6. 点击「导出工程」生成多轨 FCPXML + SRT
```
...
SKILL_DIR="/Users/xuansir/.claude/skills/videocut/剪口播"

# 2A. 默认：调用火山引擎大模型录音文件极速版 API
"/Users/xuansir/.claude/skills/videocut/.venv/bin/python" \
  "$SKILL_DIR/scripts/volcengine_bigmodel_flash_transcribe.py" \
  audio.mp3 \
  -o volcengine_result.json
# 输出: volcengine_result.json

# 2B. 备用：本地 Whisper MLX 转录（仅当火山引擎不可用时使用）
"/Users/xuansir/.claude/skills/videocut/.venv/bin/python" \
  "$SKILL_DIR/scripts/local_whisper_mlx_transcribe.py" \
  audio.mp3 \
...
- **范围整段删除**：标记口误时，从 startIdx 到 endIdx 之间的**所有元素**（含中间的 gap，不管多短）全部加入 auto_selected。不要逐个挑选文字 idx 而跳过 gap
...
`prepare_multicam_project.js` 生成，审核页直接读取。它保留原 token 字段，并新增 `mode: "multicam"`、`sources`、`speakers`、`words[].speakerId`、`words[].sourceId`。

`words[].start/end` 表示全局稿件时间，导出时再用 source offset 映射回各自素材。
